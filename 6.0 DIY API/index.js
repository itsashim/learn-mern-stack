...
app.get("/filter",(req,res)=>{
  ...
  if(!jokeType) res.status(400);

  const filteredJokes = jokes.filter((joke)=>{
    return joke.jokeType.toLowerCase() === jokeType.toLowerCase();
  })
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  if (index === -1) {
    // If no joke with the given ID is found, return a 404 response
...
app.delete("/delete/all",(req,res)=>{
  jokes = [];
  res.json({message: "Emptied successfully"});
})
